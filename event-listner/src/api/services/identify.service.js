...
import { trackedUserType, userActivityState } from "../../constant/variables.js";

const identify = async (data) => {      
    const functionName = 'identify';
    const tenantDB = $connectTenant(data.userInfo.email);
    try {
      if (data.body?.user_info?.user_type !== trackedUserType.IDENTIFIED) {
        return {
          canProceed: false,
    ...
    //   }
      ...
      if (!responseTrackAttributes.canProceed) {
        return {
          canProceed: false,
          error: responseTrackAttributes.error
        };
      }
      ...
      if (!sessionForAnonymous && !sessionForIdentified) {
        ...
        return {
          canProceed: false,
          error: 'Session not created for user'
        }
