...
class RedisService {
  ...
  async zadd(key, score, value) {
    try {
      return await this._redis.zadd(key, score, JSON.stringify(value));
    } catch (error) {
  ...
  async zrangebyscore(key, inf, threshold) {
    try {
      return await this._redis.zrangebyscore(key, inf, threshold);
    } catch (error) {
  ...
  async cache(key, callback, expiry = 360000) {
    try {
